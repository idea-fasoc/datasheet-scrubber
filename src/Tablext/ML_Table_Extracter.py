...
def space_finder(size, pixel_data, resolution): #finds white spaces
    ...
    for y in range(size[1]):
        ...
        for x in range(size[0]):
            if((pixel_data[x,y][0] + pixel_data[x,y][1] + pixel_data[x,y][2]) > resolution * 2):
                line_dist += 1
            else:
                line_dist = 0 
...
def table_locator(white_hor, table_loc):
    ...
    while(iter < moving_max_dist): #fixes the case where the the start and end are in the same spot
        if(start_array[iter] >= end_array[iter]):
            ...
            if(iter + 1 < moving_max_dist): #connect any two tables if there is no white_hor in between them
                white_in_between = False
                for w_iter in white_hor:
                    if(w_iter > end_array[iter] and w_iter < start_array[iter+1]):
                        white_in_between = True                       
                if(not white_in_between):
                    print("Table Fixed!")
                    end_array[iter] = end_array[iter+1]
                    moving_max_dist -= 1
                    del start_array[iter+1]
                    del end_array[iter+1]
            iter += 1
