...
def write_to_csv(page_num, work_loc, final_data):
    cleaned_array = []
    for small_array in final_data:
    ...
    with open(os.path.join(work_loc, ("P" + str(page_num) + ".csv")), "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerows(cleaned_array)

def run_main(page_num, image, root, work_loc, identify_model, identify_model2, conc_col_model, valid_cells_model):
    ...
    print(page_num)
    write_to_csv(page_num, work_loc, final_data) 

def multiprocessing_unit(image_num, image, root, work_loc):
    print("entered!")
    ...
    run_main(page_num, image, root, work_loc, identify_model, identify_model2, conc_col_model, valid_cells_model)
    #print(temp_array)
    #return temp_array
...
if __name__ == '__main__':
    ...
    work_loc = r"/Users/Renee/Desktop/work_output"
    ...
    for image_num, image in enumerate(pages):
        process_list.append(multiprocessing.Process(target=multiprocessing_unit, args=(image_num, image, root, work_loc))) 
