            ...
            if(len(ver_lines_final[space_iter]) > 0): #needed for segements without any verticle lines
                ...
                while(x < ver_lines_final[space_iter][-1]):
                    ...
                    if(x not in real_ver[space_iter]): #skip over verticle lines
                        ...
                        if(black_encountered > (resolution/75)):
                            #inferred_line_dist_max = 0 
                            inferred_line_dist = 0
...
def horizontal_line_finder(size, pixel_data, resolution, start, end): #normal finds black/white lines, other finds white spaces
    ...
    for iter in range(len(start)):
        for y in range(start[iter], end[iter]):
            ...
            line_dist_max = 0
            for x in range(size[0]):
                if((pixel_data[x,y][0] + pixel_data[x,y][1] + pixel_data[x,y][2]) < 255):
                    line_dist += 1
                else:
                    line_dist = 0 
...
def check_box(start, end, pixel_data):
    ...
    #print(count, " ", (end[0] - start[0]) *(end[1] - start[1]))
    useless = False
    if(count == 0):
       useless = True
...
def image_to_text(start, end, img, i, j):
    ...
    return pytesseract.image_to_string(loc, config='--psm 6')
...
def horizontal_line_split(j, x_start, x_end, hor_line, pixel_data, resolution): #TODO make scalable with resolution //Only can split 2 rows
    ...
    while(y_iter < hor_line[j+1]): #finds which lines contain black values
        black_pixel = 0
        iter = x_start
    ...
    required_thickness = resolution/30
...
def master(a, pages_in, final_data_array, resolution, table_loc, complete_csv_path):
    ...
    for n in range(len(table_start)):
        ...
        for ver_line_iter in ver_lines[n]: #look at every section between verticle lines
    ...
    #print("Inferred Ver: ", inferred_ver_lines)
    #print("Ver Lines: ", ver_lines)

    ver_lines_final = []
    guarenteed_inf_ver = []
    for n in range(len(table_start)): #Find the correct verticle lines
        guarenteed_inf_ver.append(inferred_vertical_line_finder((table_start[n], table_end[n]), (0, page.size[0]), pixel_data, resolution, .99, 10, hor_lines)) #inject inf_ver that might have been wrongfully removed; Thcker line required
        tempv = mean_finder(ver_lines[n], guarenteed_inf_ver[n], resolution/15) #TODO find a good number
        if(len(tempv) > 0):                
            ver_lines_final.append(mean_finder(tempv, ([0] + inferred_ver_lines[n] + [page.size[0]-10]), resolution/5)) #this is precision not resolution add lines to the left and right //TODO find a good precision
        else:
    ...
    while (j < len(hor_lines_final)):
        ...
        while (i < len(ver_lines_final[n])):
            ...
            if(empty): #empty
                ...
                #TODO add row combining along with row spliting

                if(split):
                    one_cell_split = True
                    data = image_to_text((ver_lines_final[n][old_x] + 1, hor_lines_final[j] + 1), (ver_lines_final[n][i+1], line_split_loc), page, i, j)    
                    cell_holder.append(image_to_text((ver_lines_final[n][old_x] + 1, line_split_loc), (ver_lines_final[n][i+1], hor_lines_final[j + 1]), page, i, j))
                else:
                    data = image_to_text((ver_lines_final[n][old_x] + 1, hor_lines_final[j] + 1), (ver_lines_final[n][i+1], hor_lines_final[j + 1]), page, i, j)
                    cell_holder.append('-')

                first_data_array.append(data)
    ...
    for n in table_start: #white spaces
        for x in range(page.size[0]):
            pixel_data[x,n] = (0,255,0)
            pixel_data[x,n+1] = (0,255,0)
            pixel_data[x,n+2] = (0,255,0)
